...
// Forked from: https://gitlab.com/idotj/mastodon-embed-feed-timeline
...
document.addEventListener("DOMContentLoaded", () => {
	let mapi = new MastodonApi({
		...
		instance_uri: 'https://mastodon.mg',
		user_id: '000004379',
		profile_name: '@barijaona',
		toots_limit: 10,
		btn_see_more: 'Lire la suite sur Mastodon'
	});
});
	...
	fetch(this.INSTANCE_URI + '/api/v1/accounts/' + this.USER_ID + '/statuses?limit=' + this.TOOTS_LIMIT, {
		...
		.then(jsonData => {
			...
			for (let i in jsonData) {
				if (jsonData[i].visibility == 'public' && !jsonData[i].in_reply_to_account_id) {
					// List only public toots and no replies (re-blogging of replies is OK)
